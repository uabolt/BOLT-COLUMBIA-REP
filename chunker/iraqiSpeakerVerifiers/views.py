...
class MultipleChoiceVerificationCreate(CreateView):
    ...
    def get(self, request):
        ...
        if qs.count() == 0:
            return super(MultipleChoiceVerificationCreate, self).get(request)
    ...
    def form_valid(self, form):
        ...
        super(MultipleChoiceVerificationCreate, self).form_valid(form)
...
def is_correct_answer_multiple_choice(form):
    ''' Defines the passing criteria for the new multiple choice prescreening
    test '''

    # This is the threshold after which we consider a test passed
    threshold = 0

    answers = form.instance.get_answers()
    ...
    return True if correct >= threshold else False
